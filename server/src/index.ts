...
function allPlotOwners(): Record<string, { ownerId: string; username: string }> {
  const rows = db.prepare(`
    SELECT po.plot_id, po.owner_id, p.username FROM plot_owners po
    JOIN players p ON p.id = po.owner_id
  `).all() as { plot_id: string; owner_id: string; username: string }[];
  const out: Record<string, { ownerId: string; username: string }> = {};
  for (const r of rows) out[r.plot_id] = { ownerId: r.owner_id, username: r.username };
  return out;
}
...
app.post('/api/plots/:plotId/claim', authMiddleware, (req: Request, res: Response) => {
  ...
  io.emit('plotUpdate', { plotId: plot.id, ownerId: player.id, username: player.username });
  res.json({ ok: true });
});
...
server.listen(PORT, () => console.log(`Old West Frontier server listening on http://localhost:${PORT}`));
