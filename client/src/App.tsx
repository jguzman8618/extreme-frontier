...
import {
  ...
  move, gather, claimPlot, buildBuilding, plantCrop, harvestCrop,
} from './api';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    s.on('plotUpdate', ({ plotId, ownerId, username }: { plotId: string; ownerId: string; username: string }) => {
      setState((prev) => (prev ? { ...prev, plotOwners: { ...prev.plotOwners, [plotId]: { ownerId, username } } } : prev));
    });
      ...
      <header className="topbar">
        ...
        <div className="stats">
          ...
          {Object.entries(state.inventory).map(([item, qty]) => (
            <span key={item} className="inv-pill">{ITEM_ICON[item] ?? '📦'} {qty}</span>
          ))}
        </div>
      </header>
        ...
        >
          {world.terrain.map((row, y) =>
            row.map((terrain, x) => {
              ...
              if (building) content = world.buildings[building.type]?.icon ?? '🏗️';
        ...
        <div className="side-panel">
          ...
          {occupyingPlot && plotOwner && (
            <p className="modal-note">
              {isMyPlot ? 'Your homestead.' : `${plotOwner.username}'s homestead.`}
            </p>
          )}
